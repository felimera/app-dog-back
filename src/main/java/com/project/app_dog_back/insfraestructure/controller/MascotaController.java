...
@Tag(name = "Mascota", description = "Pet-related operations.")
@RestController
...
public class MascotaController {
    ...
    @Autowired
    public MascotaController(IMascotaService iMascotaService, IMetaService iMetaService) {
        this.iMascotaService = iMascotaService;
        this.iMetaService = iMetaService;
    }

    @Operation(summary = "Create a pet registration.")
    @ApiResponses(value = {
    ...
    @PostMapping
    public ResponseEntity<Response> create(@Valid @RequestBody MascotaDto dto, BindingResult bindingResult) {
        log.info("Creating Mascota: {}", dto);
        ...
        response.setData(iMascotaService.create(dto));
    ...
    public ResponseEntity<Response> getById(
            @Parameter(name = "id", in = ParameterIn.PATH, description = "ID de la mascota a obtener.", required = true)
        ...
        response.setData(iMascotaService.getById(id));

        return ResponseEntity.ok(response);
    }

    @Operation(summary = "Obtain all pet records.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved"),
            @ApiResponse(
                    responseCode = "404",
    ...
    public ResponseEntity<Response> getAll() {
        ...
        response.setMeta(meta);
        response.setPagination(Pagination.builder().build().toPaginationBuilder());
        response.setData(iMascotaService.getAll());
