...
@Tag(name = "Suministro de alimento", description = "Operations related to the supply of food.")
@RestController
@RequestMapping(path = "/api/v1/suministro")
@Slf4j
@CrossOrigin(origins = "http://localhost:4200")
public class SuministroAlimentoController {
    private ISuministroAlimentoService iSuministroAlimentoService;
    private IMetaService iMetaService;

    @Autowired
    public SuministroAlimentoController(ISuministroAlimentoService iSuministroAlimentoService, IMetaService iMetaService) {
    ...
    @PostMapping
    public ResponseEntity<Response> create(@Valid @RequestBody SuministroAlimentoDto dto, BindingResult bindingResult) {
        log.info("Creating Suministro: {}", dto);
        ...
        response.setData(iSuministroAlimentoService.create(dto));
    ...
    public ResponseEntity<Response> getById(
            @Parameter(name = "id", in = ParameterIn.PATH, description = "ID de la suministro a obtener.", required = true)
        ...
        response.setData(iSuministroAlimentoService.getById(id));
    ...
    public ResponseEntity<Response> getAll() {
        ...
        response.setMeta(meta);
        response.setPagination(Pagination.builder().build().toPaginationBuilder());
        response.setData(iSuministroAlimentoService.getAll());
